...
- 更新 `知识库/进阶方法/30_Expectorpatro全站知识映射与使用边界.md` 的版本、覆盖证据和现行/历史问题状态；
- 修正 `知识库/进阶方法/27_主成分分析严谨方法卡.md` 中 PCA 分母问题的时态；
- 在 `知识库/进阶方法/32_神经网络严谨方法卡.md` 登记独立 `gru.py` 的禁止直接复用边界；
- 新增 `2026-08-12_gru附件代码审计.md`；
- 保留 `知识库/进阶方法/33_EM算法与潜变量模型严谨方法卡.md` 的原审计结论，因为用户附件与此前审计副本字节相同。
