...
6. **证据层**：`来源资料/` 保存完整台账与原始材料；`证据注册/` 保存高风险主张的机械绑定子集。
...
- 来源身份以 `来源资料/来源与证据台账.md` 为当前权威登记。
- `证据注册/source_registry.json` 与 `证据注册/claims_registry.json` 只登记已迁移的高风险主张，不取代完整来源台账。
...
- 根目录保留 Skill 入口、机器路由、永久质量核心、阶段门禁和 GitHub 标准文件；
- `知识库/基础方法/` 保存 01—19 基础方法，`知识库/进阶方法/` 保存 22—40 进阶方法卡；
- `知识库/竞赛论文/` 只保存获奖论文案例方法论，不作为默认公式证据；
- `模板/` 与 `复盘记录/` 分开保存可复用模板和任务级经验；
- `来源资料/`、`证据注册/`、`tools/` 和 `tests/` 继续承担证据、执行和验证职责。

文件移动只改变路径和导航，不改变方法正文。机器调用仍以 `routing-manifest.yaml` 为唯一事实源。
