...
- `knowledge/foundations/` 保存 01—19 基础方法，`knowledge/advanced/` 保存 22—40 进阶方法卡；
- `knowledge/competition/` 只保存获奖论文案例方法论，不作为默认公式证据；
- `templates/` 与 `retrospectives/` 分开保存可复用模板和任务级经验；
- `sources/`、`evidence/`、`tools/` 和 `tests/` 继续承担证据、执行和验证职责。
